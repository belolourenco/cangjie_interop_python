...
This project builds a Cangjie Python interop library backed by an embedded CPython bridge.
...
- Python development headers and embed linker flags available through `python3-config`.
...
`make native` builds `build/native/libpython_bridge.a` from `native/bridge/python_bridge.c`.
...
The test packages link against `libpython_bridge` and the local CPython embed library. If Python is installed somewhere else, update the `link-option` values in the test `cjpm.toml` files to match `python3-config --ldflags --embed`.
